                ...
                while ($msg = mysql_fetch_assoc($messages))
                {
                    if(!strstr($msg['data'], "/w ")){
                        if($Session->CheckPermission('Plugins.Van2Shout.Delete')) {
                            array_push($text, "<li><strong><a href=\"javascript:remove('".$msg['EntryID']."');\"><img src='/plugins/Van2Shout/rem.png' width:'20px' height:'20px' style='float:left';/></a><a style='float:right'; href='?p=profile/".urlencode($msg['Username'])."'>".$msg['Username']."</a>: ".wordwrap($msg['data'],30," ",true)."</strong></li>");
                        } else {
