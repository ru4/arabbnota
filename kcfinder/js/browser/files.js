...
browser.initFiles = function() {
    ...
    $('.file>div').unbind();
    $('.file>div').click(function(e) {
        _.unselect();
        browser.selectFile($(this), e);
    });
    $('.file>div').rightClick(function(e) {
        _.unselect();
        browser.menuFile($(this), e);
    });
    $('.file>div').dblclick(function() {
        _.unselect();
        browser.returnFile($(this));
    });
    $('.file>div').mouseup(function() {
        _.unselect();
    });
    $('.file>div').mouseout(function() {
        _.unselect();
    });
        ...
        $.each(browser.files, function(i, file) {
            if (_.kuki.get('view') == 'list') {
                ...
                html += '<div class="file">' +
					...
					'<div class="size">الرابط :</div>' +
					 '<textarea readonly="readonly" style=" width:100px;" >http://arabbnota.com' +  browser.uploadURL + '/' + browser.dir + '/' + file.name+ '</textarea>' +
                '</div>';
            }
        });
